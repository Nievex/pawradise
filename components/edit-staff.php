...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fullname) || empty($username) || empty($email) || empty($phone) || empty($role) || empty($shelter) || empty($address)) {
    ...
    if (count($errors) === 0) {
        ...
        $stmt = $conn->prepare("UPDATE staffs SET fullname = ?, username = ?, email = ?, phone = ?, address = ?, staff_img = ? WHERE id = ?");
        $stmt->bind_param(
            "sssssssssi",
